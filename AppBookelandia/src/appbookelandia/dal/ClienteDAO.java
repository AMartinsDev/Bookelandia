...
public class ClienteDAO implements ICrud<Cliente> {
    ...
    public void altera(Cliente t) throws SQLException {
        String sql = "update clientes set nome=?,telefone=?, endereço=? where cpf =?;";
        PreparedStatement ps = conexao.getConexao().prepareStatement(sql);
    ...
    /*
    public Cliente exclui(Cliente c) throws SQLException {

          //cria a string de consulta
        String sql = "delete cpf, nome, telefone, endereço, dataDeCadastro from clientes where cpf = ?;";
        //cria a declaraçao sql
        PreparedStatement ps = conexao.getConexao().prepareStatement(sql);
        //Inclui o parâmetro de consulta na declaraçao sql
        ps.setString(1, c);
